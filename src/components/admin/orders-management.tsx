    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[['All orders', counts.all], ['Pending', counts.pending], ['Processing', counts.processing], ['Shipping', counts.shipping], ['Delivered', counts.delivered]].map(([label, value]) => (
          <button key={String(label)} onClick={() => setStatus(label === "All orders" ? "ALL" : label === "Processing" ? "PROCESSING" : label === "Shipping" ? "SHIPPED" : String(label).toUpperCase())} className="text-left bg-white border border-cream rounded-2xl p-4 hover:border-charcoal/30 transition">
            <p className="text-xs uppercase tracking-[0.16em] text-secondary">{label}</p><p className="text-2xl font-semibold mt-1">{value}</p>
          </button>
        ))}
      </div>

      <section className="bg-white border border-cream rounded-2xl p-4 lg:p-5 print:hidden">
        <div className="flex flex-col lg:flex-row gap-3">
          <div className="flex-1 relative"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search order #, customer, email or tracking…" className="w-full rounded-xl border border-cream bg-cream/30 px-4 py-3 outline-none focus:ring-2 focus:ring-charcoal/10" /></div>
          <select value={status} onChange={e => setStatus(e.target.value)} className="rounded-xl border border-cream px-4 py-3 bg-white"><option value="ALL">All statuses</option>{Object.entries(statusLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>
          <select value={payment} onChange={e => setPayment(e.target.value)} className="rounded-xl border border-cream px-4 py-3 bg-white"><option value="ALL">All payment states</option><option value="UNPAID">Unpaid</option><option value="PAID">Paid</option><option value="FAILED">Failed</option><option value="REFUNDED">Refunded</option></select>
          <select value={sort} onChange={e => setSort(e.target.value as typeof sort)} className="rounded-xl border border-cream px-4 py-3 bg-white"><option value="newest">Newest first</option><option value="oldest">Oldest first</option><option value="highest">Highest value</option></select>
          <button onClick={() => exportCsv(filtered)} className="rounded-xl border border-cream px-4 py-3 font-medium hover:bg-cream">Export CSV</button><button onClick={printList} className="rounded-xl border border-cream px-4 py-3 font-medium hover:bg-cream">Print list</button>
        </div>
      </section>

      <section className="bg-white border border-cream rounded-2xl overflow-hidden">
        <div className="hidden md:grid grid-cols-[1.1fr_1.4fr_.9fr_1fr_1fr_1.2fr_auto] gap-4 px-5 py-3 bg-cream/60 text-[11px] uppercase tracking-[0.14em] text-secondary font-medium"> <span>Order</span><span>Customer</span><span>Total</span><span>Status</span><span>Payment</span><span>Date</span><span /></div>
