...
export default function ProductForm({ mode, productId, initialData = emptyProductForm, initialVariants = [] }: Props) {
  ...
  const [categories, setCategories] = useState<Array<{ id: string; name: string; parentId: string | null; active: boolean; sortOrder: number }>>([])

  useEffect(() => {
    fetch("/api/admin/categories", { cache: "no-store" }).then(r => r.ok ? r.json() : []).then(data => setCategories(Array.isArray(data) ? data : [])).catch(() => setCategories([]))
  }, [])

  const mainCategories = useMemo(() => categories.filter(c => !c.parentId && c.active), [categories])
  const subcategories = useMemo(() => categories.filter(c => c.parentId === categories.find(c => c.name === formData.category && !c.parentId)?.id && c.active), [categories, formData.category])
    ...
    <form onSubmit={submit} className="space-y-6">
      <section className="bg-white rounded-xl border border-cream p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
          ...
          <Select label="Category *" value={formData.category} onChange={v => { update("category", v); update("subcategory", "") }} options={(mainCategories.length ? mainCategories.map(c => c.name) : ["Women", "Men", "Kids", "Luxury", "Accessories"])} required />
          {subcategories.length > 0 ? <Select label="Subcategory" value={formData.subcategory} onChange={v => update("subcategory", v)} options={subcategories.map(c => c.name)} allowEmpty /> : <Field label="Subcategory" value={formData.subcategory} onChange={v => update("subcategory", v)} placeholder="Ready to Wear / Unstitched" />}
