    ...
    try {
      const response = await fetch("/api/coupons", {
        ...
        body: JSON.stringify({
          ...
          applicableCategories: formData.applicableCategories.split(",").map((v: string) => v.trim()).filter(Boolean),
          applicableProductIds: formData.applicableProductIds.split(",").map((v: string) => v.trim()).filter(Boolean),
