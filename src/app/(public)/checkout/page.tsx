  ...
  return <main className="min-h-screen bg-[#faf9f6]">
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
       <div className="mb-7 grid gap-3 sm:grid-cols-3">
         <div className="flex items-center gap-3 border border-black/5 bg-white p-4"><ShieldCheck size={17}/><div><p className="text-xs font-medium">Secure checkout</p><p className="text-[11px] text-secondary mt-0.5">Protected order details</p></div></div>
         <div className="flex items-center gap-3 border border-black/5 bg-white p-4"><Truck size={17}/><div><p className="text-xs font-medium">Pakistan-wide</p><p className="text-[11px] text-secondary mt-0.5">Reliable delivery</p></div></div>
         <div className="flex items-center gap-3 border border-black/5 bg-white p-4"><LockKeyhole size={17}/><div><p className="text-xs font-medium">COD only</p><p className="text-[11px] text-secondary mt-0.5">Pay when delivered</p></div></div>
       </div>
      ...
      <div className="grid lg:grid-cols-[1fr_390px] gap-8 lg:gap-12 items-start">
        <section>
          ...
          <form id="noore-checkout-form" onSubmit={placeOrder} className="bg-white border border-black/5 p-5 md:p-7">
    ...
    {step === 3 && <div className="fixed inset-x-0 bottom-0 z-40 border-t border-black/10 bg-white/95 p-3 shadow-2xl backdrop-blur md:hidden">
      <button type="submit" form="noore-checkout-form" disabled={loading} className="mx-auto flex h-12 w-full max-w-7xl items-center justify-center bg-charcoal text-xs font-medium uppercase tracking-[0.14em] text-white disabled:opacity-50">
        {loading ? "Placing order…" : `Place COD order · PKR ${grandTotal.toLocaleString()}`}
      </button>
    </div>}
  </main>
}
