...
export default function OrderConfirmationPage() {
  ...
  useEffect(() => {
    ...
    if (redirectStatus === "succeeded") {
      ...
      // For COD orders or direct success
      setPaymentStatus("success")
    }

    // Get order number from localStorage
    const storedOrder = localStorage.getItem("last_order_number")
    if (storedOrder) {
        ...
        <div className="bg-white rounded-lg border border-cream p-8 text-center">
          {paymentStatus === "success" ? (
            ...
            <>
              ...
              <div className="flex gap-4 justify-center mt-8">
                <Link href="/checkout" className="bg-charcoal text-white px-6 py-3 rounded font-medium hover:bg-charcoal/80 transition">
                  Try Again
                </Link>
                <Link href="/cart" className="border border-charcoal text-charcoal px-6 py-3 rounded font-medium hover:bg-charcoal hover:text-white transition">
                  Return to Cart
                </Link>
              </div>
            </>
