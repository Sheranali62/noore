...
export async function PATCH(
  ...
  try {
    ...
    const status =
      body.status as typeof ORDER_STATUSES[number] | undefined

    const paymentStatus =
      body.paymentStatus as typeof PAYMENT_STATUSES[number] | undefined
    ...
    const order = await prisma.$transaction(async (tx) => {
      ...
      // return ordered quantity back into inventory.
          ...
          if (item.variantId) {
            ...
            await tx.inventoryMovement.create({
              data: {
                ...
                beforeStock: currentVariant.stock,
                afterStock:
            ...
            await tx.inventoryMovement.create({
              data: {
                productId: item.productId,
                change: item.quantity,
                beforeStock: currentProduct.stock,
                afterStock:
      ...
      // Reopening a cancelled/returned/refunded order:
      // remove the quantity from inventory again.
      if (wasCancelled && !willBeCancelled) {
        for (const item of current.items) {
          if (item.variantId) {
            const currentVariant =
